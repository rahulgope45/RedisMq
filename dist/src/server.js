...
app.get('/add-job', async (req, res) => {
    ...
    await redisClient.lPush('emailQueue', JSON.stringify(job));
    res.send('Job Added');
});
app.get('/jobs', async (req, res) => {
    const jobs = await redisClient.lRange('emailQueue', 0, -1);
    res.send(jobs);
});
app.get('/process-job', async (req, res) => {
    const jobs = await redisClient.lPop('emailQueue');
    if (!jobs) {
